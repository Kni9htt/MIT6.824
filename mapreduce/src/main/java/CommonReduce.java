            ...
            stream.forEach(
                 s -> {
                     ...
                     KeyValue keyValue = JSON.parseObject(
                             ...
                             KeyValue.class);
                     List<String> keyValues = kvsMap.getOrDefault(keyValue.getKey(), new ArrayList<>());
                     keyValues.add(keyValue.getValue());
                     kvsMap.put(keyValue.getKey(), keyValues);
                 });
        }
        List<String> keys = new ArrayList<>(kvsMap.keySet()).stream().sorted(String::compareTo).collect(Collectors.toList());
        keys.forEach(k -> {
