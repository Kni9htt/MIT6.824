...
public class Worker extends RpcNode {
    ...
    public void work (MapFunc mapFunc, ReduceFunc reduceFunc) {
        ...
        while (mrArg.getWorkerId() != null && !mrArg.getDone()) {
            if (mrArg.getWorkerType().equals(Cons.TASK_TYPE_MAP)) {
                commonMap.doMap(mapFunc, mrArg.getWorkerId(), mrArg.getJobFile(), mrArg.getReduceNum());
            }
            if (mrArg.getWorkerType().equals(Cons.TASK_TYPE_REDUCE)) {
                commonReduce.doReduce(reduceFunc, mrArg.getWorkerId(), mrArg.getMapNum(), mrArg.getReduceOutFile());
            }
    ...
    private MRArg doneTask(Integer id, Integer type, Integer port) {
        return JSON.parseObject(call(Cons.MASTER_HOST, "doneTask", new Object[]{id, type, port}).toString(), MRArg.class);
    }

    private MRArg requireTask(Integer port) {
        MRArg arg = JSON.parseObject(call(Cons.MASTER_HOST, "requireTask", new Object[]{port}).toString(), MRArg.class);
        return arg;
    }
